...
def colourstrip_optimized(data: str) -> str:
    ...
    # This matches \x03, \x0312, \x0312,34 but keeps commas if there is following number
...
def is_socket(xs: trio.SocketStream | trio.SSLStream) -> bool:
    ...
    """
    if not isinstance(xs, trio.SocketStream) and not isinstance(xs, trio.SSLStream):
        return False
...
def get_words(text: str) -> list:
    ...
    lower_string = remove_newline(lower_string)
...
async def identd_handler(server_socket: trio.SocketStream) -> None:
    ...
    try:
        data = await server_socket.receive_some(300)
        await server_socket.send_all(
            data.strip() + b" : USERID : UNIX : MyProxyIP\x0c\n"
        )
    except (Exception, ExceptionGroup) as e:
        print("identd error: " + str(e.args))


async def proxy_make_irc_connection(
    client_socket: trio.SocketStream | trio.SSLStream, ss_hostname: str, port: int
) -> None:
    ...
    server_socket: trio.SSLStream | trio.SocketStream
    try:
        granted: bytes = b"HTTP/1.0 200 connection started with irc server.\x0c\n\x0c\n"
        ...
        with trio.CancelScope() as scope2:
            async with trio.open_nursery() as nursery:
                try:
                    ...
                    nursery.start_soon(
                        socket_received_chunk, client_socket, server_socket, "cs"
                    )
                    # Write to client
                    nursery.start_soon(
                        write_loop,
                        client_socket,
                        server_socket,
                        socket_data.send_buffer[client_socket],
                        "cs",
                    )
                    # Write to server
                    nursery.start_soon(
                        ...
                        client_socket,
                        server_socket,
                        socket_data.send_buffer[server_socket],
                        "ss",
                    )
...
async def make_normal_socket(
    client_socket, ss_hostname: str, port: int
) -> trio.SocketStream | trio.SSLStream | None:
    """Setup normal socket"""
...
async def write_loop(
    ...
    send_buffer: Deque[str | bytes | str],
    which_sock: str,
) -> None:
    ...
    line: bytes = b""
    line_count: int = 0
    while (client_socket in socket_data.mysockets) and (
        server_socket in socket_data.mysockets
    ):
        if not line:
            try:
...
async def script_command(client_socket, server, line):
    ...
    if low_line[1] == "nick":
        if (
            await UserCommands.user_nick_change(client_socket, server, source, target)
            is True
        ):
            return True
    if low_line[1] == "part":
    ...
    if (
        source
        and len(low_line) >= 4
        and len(low_line[3]) >= 3
        and low_line[1] == "privmsg"
        and low_line[3][1] == "."
    ):
        ...
        if len(low_line) >= 5:
            ...
            parms = ""
        await UserCommands.execute_user_command(
            client_socket, server, source_nick, target, cmd, parms
        )
        return True
    else:
        source_nick = ""
...
def cs_away_msg_notify(
    client_socket: trio.SocketStream | trio.SSLStream, nick: str, msg: str
) -> None:
    ...
    if not msg:
        ...
        parm = away
    actions.cs_send_notice(client_socket, "User " + nick + " is set " + parm)
    return None
...
async def socket_received_chunk(
    client_socket: trio.SocketStream | trio.SSLStream,
    server_socket: trio.SocketStream | trio.SSLStream,
    which_sock,
) -> None:
    ...
    if "connecting" == socket_data.state[client_socket]["doing"]:
        ...
        socket_data.echo(
            client_socket, "connected to " + socket_data.hostname[server_socket]
        )
    ...
    while True:
        ...
        while True:
            ...
            read_some_found = (
                find_n + 1
            )  # increase by one to read the find_n-1 character
            read_some = read_line[
                0 : find_n + 1
            ]  # increase by one to account for 0 start
        ...
        if find_n == -1:
            ...
            if read_line:
                ...
                await trio.sleep(0.30)
    return None


async def ss_received_line(
    client_socket: trio.SocketStream | trio.SSLStream,
    server_socket: trio.SocketStream | trio.SSLStream,
    single_line: str,
    split_line: list[str],
    ...
    if len(split_line) == 0:
        actions.sc_send(dest_socket, "\x0c\n")
        return
    ...
    if original_line[0] == "@":
        if (
            not single_line.startswith("@time")
            and not single_line.startswith("@account")
            and not single_line.startswith("@batch")
            and not single_line.startswith("@msgid")
        ):
    ...
    ial_send = await ss_updateial(
        client_socket, server_socket, single_line, orig_upper_split
    )
    ...
    if split_line[0] == "376":
        ...
        reason = " ".join(orig_upper_split[4:]).strip(":\x0c\n\t\f ")
        msg = (
            f":{NAKED_URL} NOTICE {socket_data.mynick[client_socket]}"
...
def sock_005(
    client_socket: trio.SocketStream | trio.SSLStream, single_line: str
) -> None:
...
def send_motd(client_socket: trio.SocketStream | trio.SSLStream, mynick: str) -> None:
    ...
    actions.sc_send(
        client_socket,
        prefix + "To connect to a SSL port, " "\x02DO NOT\x02 prefix the port with -",
    )
    actions.sc_send(
        client_socket,
        prefix + "a \x02+\x02 character. End-to-end encryption"
        " is not possible; but SSL from proxy to irc server is. -",
    )
    actions.sc_send(
        client_socket, prefix + "Trio-ircproxy.py \x02will\x02 use SSL for " "irc -"
    )
    actions.sc_send(client_socket, prefix + "server connections on specific ports. -")
    actions.sc_send(client_socket, prefix)
    actions.sc_send(
        client_socket,
        prefix + "Type \x02/proxy-commands\x02 for list of valid " "commands. -",
    )
    actions.sc_send(
        client_socket,
        prefix + "Type \x02/xdcc-commands\x02 for list of xdcc " "commands. -",
    )
    actions.sc_send(
        client_socket,
        prefix + "Type \x02/proxy-help\x02 for first use " "instructions. -",
    )
    actions.sc_send(client_socket, prefix)
    actions.sc_send(
        client_socket, prefix + "To view the irc server's MOTD type " "\x02/motd\x02 -"
    )
    actions.sc_send(
        client_socket,
        prefix + "Trio-ircproxy.py and Machine-Gun mSL script" " official website: -",
    )
    actions.sc_send(
        client_socket, prefix + "W-W-W: \x1fhttps://www.MyProxyIP.com/\x1f -"
    )
    actions.sc_send(
        client_socket, prefix + "'X-Clacks-Overhead':\x02 'GNU Terry Pratchett' \x02-"
    )
...
def cs_rcvd_command(
    ...
    if len(split_line) >= 2 and split_line[2] == ":.colour":
        actions.sc_send(
            client_socket,
            ":ashburry!mg-script@www.myircproxyip.com privmsg ashburry :colour is: "
            + str(ord(split_line[1][0]))
            + " = "
            + split_line[1][0],
        )
...
def verify_login(auth_userlogin: str) -> bool | tuple[str, str]:
    ...
    try:
        ...
        if (
            auth_login.count(":") > 1
            or auth_login.count(":") < 1
            or auth_login.count(" ") > 0
...
identd_list: list[list[str, float]] = []
...
async def proxy_server_handler(cs_before_connect: trio.SocketStream) -> None:
    ...
    socket_data.echo(
        cs_before_connect,
        "Accepted a client connection " + hostname + " on port " + str(port) + "...",
    )
    ...
    if cancel_scope.cancelled_caught:
        await aclose_sockets(cs_before_connect)
        socket_data.echo(
            cs_before_connect, "Client is too slow to send data. Socket closed."
        )
        await trio.sleep(0)
        return None
    try:
        ...
        auth = False
        while b"\x0c" in byte_string_data:
            byte_string_data = byte_string_data.replace(b"\x0c", b"\n")
        while b"\n\n" in byte_string_data:
        ...
        while True:
            ...
            if i >= len(str_lines) or len(str_lines) == 0:
                break
            if (str_lines[i]) == "":
                del str_lines[i]
                continue
        ...
        if cs_before_connect not in socket_data.state:
            ...
            system_data.user_settings["by_username"][auth[0].lower()].add(
                cs_before_connect
            )
        try:
...
async def before_connect_sent_connect(
    cs_sent_connect: trio.SocketStream | trio.SSLStream, byte_string: str
) -> None:
    """The socket is in a state where the client has just sent the CONNECT
    protocol statement but has not received a reply yet.
    ...
    if len(words) < 3 or len(words) > 3:
        await cs_sent_connect.send_all(
            "HTTP/1.0 400 Bad Request. Requires proper http/1.0 protocol.\x0c\n\x0c\n".encode()
        )
    ...
    if words[0] != "connect":
        await cs_sent_connect.send_all(
            "HTTP/1.0 400 Bad Request. Proxy use only for IRC networks.\x0c\n\x0c\n".encode()
        )
    ...
    if ":" not in host:
        await cs_sent_connect.send_all(
            "HTTP/1.0 400 Bad Request. Requires `server:port` to connect to.\x0c\n\x0c\n".encode()
        )
    ...
    try:
        ...
        await cs_sent_connect.send_all(
            "HTTP/1.0 400 bad request. requires integer port number.\x0c\n\x0c\n".encode()
        )
...
async def start_proxy_listener():
    ...
    if system_data.Settings_ini.has_section("settings") is False:
        system_data.Settings_ini.add_section("settings")
    ...
    try:
        ...
        if (
            len(exc.args) > 1
            and ("98" in str(exc.args[0]) or exc.args[0] == 10048)
            or (len(exc.args) >= 2 and len(exc.args[1]) > 0)
            and ("98" in str(exc.args[1][0]) or "10048" in str(exc.args[1][0]))
        ):
            print(
