...
from . import actions
from .xdcc_link import XdccBot, XdccBotSLL
...
def cs_send_notice(client_socket: trio.SSLStream | trio.SocketStream, msg):
    ...
    if not client_socket:
        return
    msg = (
        ":ashburry.pythonanywhere.com NOTICE "
...
class Xdcc_System:
    ...
    xdcc_bot_list.add_pack(
        "ashburry", "#5ioE", "109G", "#521 Bauderr mSL script - Ashburry"
    )
    ...
    xdcc_bot_list.add_pack(
        ...
        "#channel",
        "10M",
        "#11 Trio_ircproxy.py with Bauderr mSL script - Ashburry",
    )
    xbot32 = XdccBot("ash540", "#5ioE-w3", "Undernet")
    xdcc_bot_list.add_pack(
        "ash540", "#5ioE-w3", "540M", "#66 Trio_ircproxy.py - Ashburry"
    )
...
def xdcc_commands(
    ...
    if "xdcc-add" in split_line[0]:
        ...
        if "=" in single_line_mod and len(split_line) >= 3:
            ...
            split_line_chat = (
                " ".join(split_line).split("=")[0].split(" ")[-1]
                + "="
                + " ".join(split_line).split("=")[1].split(" ")[0]
            )
        ...
        if len(split_line) == 3:
            ...
            cs_send_notice(
                ...
                + "channel {} on the correct irc network to begin the indexing".format(
                    split_line[1]
                ),
            )
            Xdcc_System.xdcc_www_chans.add(split_line[1])
