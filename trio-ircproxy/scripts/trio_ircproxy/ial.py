...
class IALData:
    ...
    @classmethod
    def sendwho(
        cls, server_socket: trio.SocketStream | trio.SSLStream, th: Timer, chan: str
    ):
        print("WHO SENT: ", chan)
    ...
    @classmethod
    def comchans_get_set(
        cls, client_socket: trio.SocketStream | trio.SSLStream, nick: str
    ) -> frozenset:
    ...
    @classmethod
    def ial_get_masks(
        cls, client_socket: trio.SocketStream | trio.SSLStream, mask: str
    ) -> frozenset:
    ...
    @classmethod
    def ial_count_nicks(
        cls, client_socket: trio.SocketStream | trio.SSLStream, chan: str
    ):
        count: int = 0
    ...
    @classmethod
    def ial_remove_nick(
        cls, client_socket, old_nick, chans: list[set, str | None]
    ) -> None:
        ...
        try:
            ...
            if chans is not None:
                if (
                    isinstance(chans, set)
                    or isinstance(chans, list)
                    or isinstance(chans, tuple)
                ):
                    for chan in chans:
