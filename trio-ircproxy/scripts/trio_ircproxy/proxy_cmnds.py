...
from ..website_and_proxy.users import add_new_user
from . import actions
...
def commands(client_socket, server_socket, single_line, split_line) -> None:
    ...
    if fnmatch(command, "*new*user*"):
        if len(split_line) < 4 or len(split_line) > 4:
            actions.sc_send(
                client_socket, "Syntax: /proxy-new-user <username> <password> <email>"
            )
            try:
...
"""
if fnmatch(command, "*up*time*"):
    if (
        client_socket in socket_data.state
        and "connected" in socket_data.state[client_socket]
        ):
            newctime = time() - socket_data.state[client_socket]["connected"]
            circular.sc_send(
                client_socket,
                "You have been connected for \x02"
                + dur_replace(duration(seconds=newctime).in_words()),
            )
            if fnmatch(command, "sh*away*nick*"):
                system_data[current_user] = user_settings
                # SHow away nick, notice privmsg off
...
circular.status_msg(
client_socket, "Syntax: /proxy-login <username> <password>"
)
return None
try:
if not verify_user_pwdfile(split_line[1], split_line[2]):
circular.status_msg(
client_socket, "Bad Login. Incorrect username or password."
)
else:
circular.status_msg(
client_socket,
f'Login accepted. You are now logged in as "{split_line[1]}"',
)
