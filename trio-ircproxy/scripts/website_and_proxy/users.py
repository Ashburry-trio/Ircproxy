...
def status_msg(client_socket: trio.SocketStream | trio.SSLStream, msg: str) -> None:
    ...
    msg = (
        ":*Proxy!trio-ircproxy.py@www.myproxyip.com PRIVMSG "
        + SocketData.mynick[client_socket]
        + " "
        + msg
        + "\r\n"
    )
...
def validate_login(name: str, email: str, password: str):
    ...
    if len(name) > 20 or len(password) > 20:
        raise ValueError(
            "Username and Password must not exceed 20 characters in length each."
        )
    if (":" or " " or "*" or "?" in password) or (":" or " " or "*" or "?" in name):
        raise ValueError(
            "colon (:), star (*), question mark "
    ...
    if (
        len(email) < 3
        or len(email.split("@")[0]) < 1
        or len(email.split("@")[1]) < 1
        or email.count("@") > 1
        or email.count("@") < 1
    ):
    ...
    *,
    ...
    try:
        login: str = (
            ...
            + account_power.lower()
            + ":"
            + sha256(bytes(password.encode("utf8"))).hexdigest()
        )
...
def admin_echo(
    client_socket: trio.SocketStream | trio.SSLStream, msg: str | bytes
) -> None:
